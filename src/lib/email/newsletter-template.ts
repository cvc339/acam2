 ...
 * Portado de: radar-ambiental/supabase/functions/radar-enviar-email/index.ts
 */
...
function limpar(texto: string): string {
  return texto
    ...
    .replace(/&[a-z]+;/gi, " ")
    .replace(/\bhttps?:\/\/\S+/gi, "")
    .replace(/\b(?:p|a|div|span|img|br|class|style|href|src|width|height|alt|text-align|center|font-size|font-weight|color|margin|padding|border|background|display|vertical-align)\b\s*[:=]?\s*/gi, "")
    .replace(/[{}]/g, " ")
    .replace(/\s+/g, " ")
    .trim()
}
...
function resumoDistinto(titulo: string, resumo: string | null): string {
  if (!resumo) return ""
  const resumoLimpo = limpar(resumo)
  const tituloLimpo = limpar(titulo)
  if (tituloLimpo.toLowerCase().includes(resumoLimpo.toLowerCase().slice(0, 60))) return ""
  if (resumoLimpo.toLowerCase().includes(tituloLimpo.toLowerCase().slice(0, 60))) return ""
  return resumoLimpo
}
...
/** Card de norma federal — fundo branco uniforme */
function renderItemFederal(item: RadarItem): string {
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;border:1px solid rgba(26,58,42,0.1);">
            <tr>
              <td style="padding:20px;">
                ...
                ${ementaFinal ? `<p style="font-family:Arial,sans-serif;font-size:12px;color:#666666;margin:0;line-height:1.5;">${esc(ementaFinal.slice(0, 250))}</p>` : ""}
              </td>
            </tr>
          </table>
        </td>
...
/** Card de norma estadual — com avatar circular do órgão */
...
function renderItemNoticia(item: RadarItem, idx: number): string {
  ...
  const metaLine = metaPartes.length > 0
    ? `<p style="font-family:Arial,sans-serif;font-size:10px;color:#999999;margin:4px 0 0;font-style:italic;">${metaPartes.join(" &middot; ")}</p>`
    : ""
